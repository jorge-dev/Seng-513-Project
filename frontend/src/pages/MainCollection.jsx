...
export default function MainCollection() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                const result = await axios.get(`/api/products/category/${slug}`);
    ...
    // Content data
    const pageTitleString = `Collections/${slug.charAt(0).toUpperCase() + slug.slice(1)}`;
    const mainCategoryTitle = slug.toUpperCase();
            ...
            <Row>
                <Col md={2}>
                    ...
                    <Button
                        ...
                        variant="secondary"
                        onClick={function(evt){setFilter("No selected filter"); setProducts(products); setNumberProducts(products.length)}}>
                            Reset Filter
                    </Button>
